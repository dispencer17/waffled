...
- **Dark mode (web/kiosk + iPhone/iPad) + color themes (web/kiosk)** — a warm dark theme
  ...
  **Settings → Display & Kiosk** (Light / Dark / Match system / Follow the sun + a color-theme
  ...
  iOS: dark-aware `Color(light:dark:)` tokens re-resolved by UIKit on the appearance change),
  with shared semantic success/danger/warn/info tokens and theme-aware calendar-event tinting
  so events pass contrast in dark mode.
