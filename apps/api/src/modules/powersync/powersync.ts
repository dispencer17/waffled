...
// Where the browser should open its sync stream. PowerSync is published on its
// own host port (not behind Caddy), so we can't just reuse the request origin —
// we keep the host the device actually reached us on and swap in that port.
//
// A fixed default (it used to be http://localhost:8090) is only ever correct on
// the server itself: every other device — kiosk tablet, phone — resolves
// localhost to ITSELF, fails to connect, and silently degrades to REST-only with
// no realtime and no offline cache. Deriving per request fixes that everywhere
// and survives the server's DHCP address changing. POWERSYNC_PUBLIC_URL still
// wins when set, for deployments that front PowerSync with its own hostname/TLS.
...
export function powerSyncPublicUrl(req: Request): string {
  ...
  const proto = h['x-forwarded-proto']?.split(',')[0].trim() || 'http'
  const rawHost = h['x-forwarded-host']?.split(',')[0].trim() || h.host || 'localhost'
  // Drop the app's port (keep IPv6 brackets intact), then apply PowerSync's.
