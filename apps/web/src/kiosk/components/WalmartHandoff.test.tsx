...
// Guard: with credentials configured the existing Walmart matching flow is untouched.
describe('WalmartHandoff (configured Walmart flow)', () => {
  const CART_URL = 'https://affil.walmart.com/cart/addToCart?items=w1'

  function mockMatch() {
    globalThis.fetch = vi.fn(async (url: string) => {
      const u = String(url)
      if (u.includes('/api/shopping/walmart/match')) {
        return {
          ok: true,
          json: async () => ({
            matched: [{
              listItemId: 'i1', name: 'Milk', quantity: 1, walmartItemId: 'w1',
              title: 'Great Value Whole Milk', priceCents: 349, thumbnailUrl: null,
  ...
  it('still matches the list and offers the pre-filled cart link + QR', async () => {
    mockMatch()
    render(<WalmartHandoff onClose={() => {}} />)
    expect(screen.getByText('Send to Walmart')).toBeInTheDocument()
    const link = await screen.findByRole('link', { name: 'Open Walmart cart' })
    expect(link).toHaveAttribute('href', CART_URL)
    await waitFor(() => expect(QRCode.toDataURL).toHaveBeenCalledWith(CART_URL, expect.anything()))
  })
})
