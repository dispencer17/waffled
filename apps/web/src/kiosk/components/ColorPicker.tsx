...
// The classic 8 member swatches — shared by the member editor, the own-profile
...
// Swatch row + a free "custom" picker (native color input behind a swatch-sized
// button), so colors aren't limited to the 8 presets.
export function ColorPicker({ value, onChange, size = 30 }: { value: string | null; onChange: (hex: string) => void; size?: number }) {
  ...
  const swatch = (c: string, selected: boolean): React.CSSProperties => ({
    width: size,
    height: size,
    borderRadius: 999,
    background: c,
  ...
  return (
    <div style={{ display: 'flex', gap: 9, flexWrap: 'wrap', alignItems: 'center', position: 'relative' }}>
      ...
      >
        {!isCustom && <span style={{ color: '#fff', fontWeight: 800, fontSize: size * 0.45, textShadow: '0 1px 2px rgba(0,0,0,.45)', lineHeight: 1 }}>＋</span>}
      </button>
      <input
        ref={customRef}
        type="color"
