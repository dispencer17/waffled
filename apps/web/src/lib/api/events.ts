...
// drives state (live + works offline) — but only while the replica is TRUSTED
// (completed a full sync and the engine isn't stalled; see sync-health). An
// untrusted replica paints at most until REST lands: a wedged engine once left
// an empty replica in charge and the kiosk rendered a blank calendar while REST
// had the real data (incident 2026-07-20). REST is the baseline for the first
// paint, for REST-only mode, and whenever trust is lost; a REST failure never
// blanks rows that are already on screen.
function useAgendaFeed(
  ...
  // Latest closures without re-running the effects (their identity changes per render).
  ...
  // Local-first: stream agenda rows straight from the local DB. A trusted
  // replica takes over from REST; an untrusted one only fills in before REST.
  useEffect(() => {
  ...
  // When replica trust flips (engine stalls or recovers), re-arbitrate: hand the
  // wheel back to REST immediately on a stall; a recovery re-takes over on the
  // replica's next emission.
