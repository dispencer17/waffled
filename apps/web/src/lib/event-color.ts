// Event → color resolution, family-aware. An event whose participants cover the
// whole household renders in the household's *family* color (customizable in
// Settings → Display, stored in settings.display.familyColorHex) instead of the
// first participant's color — so the calendar reads at a glance: everyone /
// some of us / one person / unassigned.
...
export function familyColorHex(household: Household | null | undefined): string {
  ...
  return typeof v === 'string' && /^#[0-9a-fA-F]{6}$/.test(v) ? v : DEFAULT_FAMILY_COLOR
}

/**
 * A "family event" = its people (participants + the color owner) cover every
 * household member. One-person households never qualify — there's no
 * whole-vs-part distinction to draw.
 */
 ...
 * Falls back to plain owner-color while members/household are still loading.
 */
